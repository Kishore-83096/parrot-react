...
function GroupConversation({
  ...
  const applyMessageStatusEvent = useCallback(
    (eventPayload) => {
      ...
      const status =
        eventPayload.type === "group.message.read" ? "read" : "delivered";
  ...
  const applyMessageReactionEvent = useCallback(
    (eventPayload) => {
      if (
        eventPayload?.type !== "group.message.reaction_updated" ||
        Number(eventPayload.room_id) !==
          Number(activeConversationRef.current.roomId)
      ) {
        return;
  ...
  const loadRoomMessages = useCallback(
    async (
      ...
      try {
        const response = await getGroupRoomMessages(roomId, {
  ...
  useEffect(() => {
    const handleInboxEvent = async (event) => {
      ...
      if (eventPayload?.type === "group.message.reaction_updated") {
        applyMessageReactionEvent(eventPayload);
      }
    };
  ...
  const shouldShowConversationStart =
    groupedTimelineItems.length > 0 && !messagePagination.hasMore;
  ...
  const typingIndicatorText = useMemo(() => {
    const typingNames = typingUserIds
      .map(
        (typingUserId) =>
          participantNamesByUserId.get(Number(typingUserId)) || "Member",
      )
      .filter(Boolean);
          ...
          if (message.is_encrypted) {
            const encryptedUploadAttachments =
              attachmentEditPlan.uploadFiles.length > 0
                ? await encryptSelectedFilesForGroupMessage(attachmentEditPlan.uploadFiles, {
  ...
  const handleSelectMessageReaction = useCallback(
    async (message, reactionKey) => {
      ...
      try {
        const response = await reactToGroupMessage(selectedRoom.id, messageId, reactionKey);
  ...
  useEffect(() => {
    resizeMessageDraft();
  }, [messageDraft, resizeMessageDraft, selectedGroupRoomId]);

  const isQueuedMessageForActiveConversation = useCallback((queuedMessage) => {
    return Number(activeConversationRef.current.roomId) === Number(queuedMessage.roomId);
  }, []);
        ...
        try {
          ...
          const encryptedAttachments =
            queuedMessage.filesToSend.length > 0
              ? await encryptSelectedFilesForGroupMessage(
          ...
          const response = await sendGroupMessage(queuedMessage.roomId, sendPayload);
          const messageResult = response.data?.result || response.data;
          const sentMessage = messageResult?.message
            ? await decryptGroupMessageForUser(messageResult.message, user)
            : null;
    ...
    return (
      <div className="parent-layout-page__room-placeholder parent-layout-page__room-placeholder--chat">
        ...
        <div className="parent-layout-page__room-placeholder-copy">
          <h3>Group chat</h3>
          <p>
            Send encrypted group messages, media files, voice notes, replies,
            and reactions to every active member.
          </p>
        </div>
        <ul className="parent-layout-page__room-feature-list" aria-label="Chat features">
          ...
          <li>Group replies and reactions</li>
        </ul>
      </div>
    );
  }
          ...
          <>
            ...
            {shouldShowConversationStart ? (
              <ConversationStartMarker>
                You have reached the beginning. This is where this group conversation started.
              </ConversationStartMarker>
            ) : null}
