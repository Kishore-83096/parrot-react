...
export const E2EE_MESSAGE_TYPE = "e2ee.group_message";
export const E2EE_MESSAGE_VERSION = 1;

const E2EE_MESSAGE_AAD = "parrot:e2ee.group_message:v1";
const ENCRYPTED_MESSAGE_PREVIEW = "Encrypted group message";
const DECRYPTION_FAILED_TEXT = "Unable to decrypt group message";
const GROUP_DEVICE_CACHE_TTL_MS = 10000;
const groupDeviceCache = new Map();
...
export function clearGroupE2EEMessageRuntimeCaches() {
  groupDeviceCache.clear();
}
...
async function getCachedGroupDevices(roomId, user) {
  const roomScope = String(roomId || "").trim();

  if (!roomScope) {
    return [];
  }

  const cacheKey = `${getCurrentUserCacheScope(user)}:room:${roomScope}`;
  const now = Date.now();
  const cachedEntry = groupDeviceCache.get(cacheKey);
  ...
  const promise = (async () => {
    const groupResponse = await getGroupCryptoDevices(roomScope);

    return getDevicesFromResponse(groupResponse)
      .map(normalizeDevice)
      .filter(Boolean);
  })().catch((error) => {
    if (groupDeviceCache.get(cacheKey)?.promise === promise) {
      groupDeviceCache.delete(cacheKey);
    }

    throw error;
  });

  groupDeviceCache.set(cacheKey, {
    expiresAt: now + GROUP_DEVICE_CACHE_TTL_MS,
    promise,
  });
...
export async function encryptGroupMessageText({
  attachments = [],
  roomId,
  text,
  user,
  ...
  if (!roomId) {
    throw new Error("Group room is required.");
  }
  ...
  const groupDevices = await getCachedGroupDevices(roomId, user);

  if (groupDevices.length === 0) {
    throw new Error("No group member has enabled encrypted messaging yet.");
  }
...
export async function decryptGroupMessageForUser(message, user, decryptionContext) {
  if (!message) {
    return message;
  }

  const replyTo = message.reply_to
    ? await decryptGroupMessageForUser(message.reply_to, user, decryptionContext)
    : message.reply_to;
...
export async function decryptGroupMessagesForUser(messages, user) {
  const messageList = Array.isArray(messages) ? messages : [];
  const decryptionContext = await getMessageDecryptionContext(user);

  return Promise.all(
    messageList.map((message) =>
      decryptGroupMessageForUser(message, user, decryptionContext),
    ),
  );
}

export async function decryptGroupRoomsForUser(rooms, user) {
  ...
  return Promise.all(
    roomList.map(async (room) => {
      ...
      return {
        ...room,
        last_message: await decryptGroupMessageForUser(
          room.last_message,
          user,
          decryptionContext,
        ),
